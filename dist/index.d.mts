...
type FieldType<T> = T extends DateField ? string : T extends DateTimeField ? string : T extends DurationField ? number : T extends MoneyField ? number : T extends NumberField ? number : T extends PercentageField ? number : T extends StringField ? string : never;
...
declare class Geckoboard {
    ...
    defineDataset<T extends Fields>(schema: {
        id: string;
        fields: T;
        uniqueBy: Schema<T>['uniqueBy'];
    }): Dataset<T>;
    ping(): Promise<void>;
}

export { Geckoboard as default };
